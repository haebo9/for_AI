...
from model.TavilyModel import TavilyModel
...
geminimodel = GeminiModel(api_key=gemini_api_key, ROOT_PATH=ROOT_PATH) 
#
tavily_api_key = os.getenv("TAVILY_API_KEY")
if tavily_api_key is None: 
    raise ValueError("TAVILY_API_KEY 환경 변수가 설정되지 않았습니다.")
tavilymodel = TavilyModel(api_key=tavily_api_key, ROOT_PATH=ROOT_PATH) 
...
class ChatModel:
    ...
    async def chat(self, message: str, session_id: str = "default_session"):
        try:
            ...
            gemini_response = geminimodel.generate_content(trimmed_content)
            if gemini_response:
    ...
    async def _call_model(self, state: State):
        ...
        gemini_response = geminimodel.generate_content(last_human_message)

        # Tavily 검색 결과 추가 (새로운 약물 입력 시에만 실행)
        tavily_search_result = ""
        if new_component:  # new_component가 비어있지 않으면 새로운 약물 입력으로 판단
            for drug in target:
                tavily_info = tavilymodel(drug)
                if tavily_info and "error" not in tavily_info:
                    tavily_search_result += f"## {drug} 관련 Tavily 검색 결과 ##\n"
                    for key, value in tavily_info.items():
                        if value:
                            tavily_search_result += f"{key}: {value}\n"
                    tavily_search_result += "\n"
                elif tavily_info and "error" in tavily_info :
                    tavily_search_result += f"## {drug} 관련 Tavily 검색 실패: {tavily_info['error']} ##\n\n"
