...
class QualityEvaluator:
    ...
    def evaluate(self, input_path: str) -> float:
        """
        입력 파일의 각 줄에 대해 품질 점수(0~1) 평균만 반환  
        각 평가 항목별 평균값은 print로 확인
        """
        ...
        with open(input_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if not isinstance(hyp, str) or not hyp.strip():
                    total_score = 0.0
                    f_score = r_score = n_score = e_score = 0.0
                else:
        ...
        result =  sum(total_scores)/len(total_scores) if total_scores else 0.0
        return result
